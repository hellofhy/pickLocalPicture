...
import com.example.fanhy.picklocalpicture.adapter.AlbumPreviewFolderAdapter;
import com.example.fanhy.picklocalpicture.adapter.AlbumPreviewImageAdapter;
import com.example.fanhy.picklocalpicture.adapter.AlbumPreviewItemDecoration;
...
import com.example.fanhy.picklocalpicture.util.AlbumLoadUtil;
 ...
 * 相册选择页面
 */
public class AlbumPreviewActivity extends AppCompatActivity implements View.OnClickListener, AlbumPreviewImageAdapter
        .OnImageItemClickListener, AlbumPreviewFolderAdapter.OnFolderItemClickListener, CompoundButton.OnCheckedChangeListener {
...
//    private int curFolderPosition;//当前folder的位置
    ...
    private void initData () {
        ...
        mAlbumPreviewImageAdapter = new AlbumPreviewImageAdapter(mContext, mImageList, curChooseImageList, maxChooseCount,
                COLUMN_COUNT);
...
//                mImageList.addAll(list.get(curFolderPosition).imageList);
    ...
    public void onClick (View v) {
        ...
        } else if (viewId == R.id.iv_mask) {//rv的蒙板
            hideAlbumSwitchFolderPopup();
        } else if (viewId == R.id.tv_submit) {//发送
            hideAlbumSwitchFolderPopup();
            returnActivityResult();
        } else if (viewId == R.id.btn_switch_folder) {//选择图库
            if (folderRvIsShow) {
                hideAlbumSwitchFolderPopup();
            } else {
                showAlbumSwitchFolderPopup();
            }
        } else if (viewId == R.id.btn_preview) {//预览
            hideAlbumSwitchFolderPopup();
    ...
    protected void onActivityResult (int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case INTENT_REQUEST_IMAGE_CODE:
                ...
                ArrayList<AlbumImageBean> albumImageList = (ArrayList<AlbumImageBean>) data.getSerializableExtra
                        (INTENT_RESULT_IMAGE_LIST);
                switch (resultCode) {
