...
import com.example.fanhy.myapplication.adapter.ImagePreviewRvAdapter;
import com.example.fanhy.myapplication.adapter.ImagePreviewVpAdapter;
import com.example.fanhy.myapplication.bean.AlbumImageBean;
import com.example.fanhy.myapplication.fragment.ImagePreviewFragment;
 ...
 * 选中图片预览页面
 */
public class ImagePreviewActivity extends AppCompatActivity implements View.OnClickListener, ImagePreviewRvAdapter.OnRvItemClickListener, ViewPager.OnPageChangeListener, ImagePreviewFragment.OnImageFragmentClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    public static void startIntentForResult(Activity activity, int requestCode, ArrayList<AlbumImageBean> imageList,
                                            ArrayList<AlbumImageBean> curChooseImageList, int maxChooseCount, int curPosition, boolean isOriPicture) {
    ...
    private void updateTvSubmit() {
        ...
        tvSubmit.setTextColor(curChooseCount <= 0 ? mContext.getResources().getColor(R.color.red) : mContext.getResources().getColor(R.color.green));
    }
